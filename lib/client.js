...
function init(type = 'browser', options = {}) {
    if(_initialized)
        return;

    const initSentry = function(Sentry) {
    ...
    if(type === 'browser')
        import('@sentry/browser').then(initSentry);

    if(type === 'react')
        import('@sentry/react').then(initSentry);
}
...
SentryLogger.init = initBrowser;
SentryLogger.initBrowser = initBrowser;
SentryLogger.initReact = initReact;
